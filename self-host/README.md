...
| `lexer.res` + `lexer_tests/` + `lexer_check.sh` | RES-323 | Production-step-up lexer covering the verification surface (requires/ensures/invariant/assume/assert/...), block comments, escapes, hex/bin literals, and 2/3-char operators. |
...
self-hosted `lexer.res` against each one with `SELF_HOST_INPUT`
...
RZ_BIN=/path/to/rz \
  ...
  /path/to/rz self-host/lexer.res 2>/dev/null \
  | grep -v '^seed=' \
...
| Criterion | State |
|---|---|
| `self-host/lexer.res` implements the complete Resilient lexer | 🟡 — covers significantly more than the RES-196 prototype (verification keywords, escapes, hex/bin, block comments, 3-char ops); full coverage matching every example in `resilient/examples/` is a follow-up |
...
| `resilient run self-host/lexer.res -- <input_file>` | ✅ — `SELF_HOST_INPUT` env var; CLI arg passing isn't a current language surface, env var is the idiomatic substitute |
