...
impl Formatter {
    ...
    fn fmt_stmt(&mut self, node: &Node) {
        match node {
            ...
            // RES-222: `invariant <expr>;` — same surface shape as assert.
    ...
    fn fmt_expr(&mut self, node: &Node) {
        match node {
            ...
            // RES-221: string interpolation — emit the canonical `"..{expr}.."` form.
            Node::StringInterpolation { parts, .. } => {
                self.write("\"");
                for part in parts {
                    match part {
                        crate::StringPart::Literal(s) => {
                            self.write(&escape_string(s).replace('{', "\\{"));
                        }
                        crate::StringPart::Expr(expr) => {
                            self.write("{");
                            self.fmt_expr(expr);
                            self.write("}");
                        }
