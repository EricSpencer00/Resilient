...
impl Formatter {
    ...
    fn fmt_stmt(&mut self, node: &Node) {
        match node {
            ...
            Node::Extern { .. } => {}
            // RES-208 (concurrency-preview): emit a source-preserving
            // placeholder comment.  The formatter is not wired up for
            // the full concurrency syntax yet — that lands in a
            // follow-on ticket once the scheduler is in.  A comment
            // is the safest lossless representation that round-trips
            // through fmt_stmt without an infinite dispatch cycle.
            #[cfg(feature = "concurrency-preview")]
            Node::ActorDecl { name, .. } => {
                self.write(&format!("// concurrency-preview: actor {}", name));
                self.newline();
            }
            #[cfg(feature = "concurrency-preview")]
            Node::Spawn { actor_name, .. } => {
                self.write(&format!("// concurrency-preview: spawn {}", actor_name));
                self.newline();
            }
            #[cfg(feature = "concurrency-preview")]
            Node::Send { .. } => {
                self.write("// concurrency-preview: send");
                self.newline();
            }
            #[cfg(feature = "concurrency-preview")]
            Node::Recv { .. } => {
                self.write("// concurrency-preview: recv");
                self.newline();
            }
    ...
    fn fmt_expr(&mut self, node: &Node) {
        match node {
            ...
            // RES-208: concurrency-preview nodes are statement-like;
            // fall through to `fmt_stmt` which handles them by
            // printing a source-preserving placeholder comment so a
            // preview build's code can still pass through the formatter
            // without data loss.
            #[cfg(feature = "concurrency-preview")]
            Node::ActorDecl { .. } | Node::Spawn { .. } | Node::Send { .. } | Node::Recv { .. } => {
                self.fmt_stmt(node);
            }
        }
    }
