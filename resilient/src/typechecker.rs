...
/// RES-223: shim for `verifier_z3::format_counterexample_block`. When the
/// z3 feature is absent there is nothing to format, so the no-op variant
/// returns an empty string (callers check for empty before appending).
#[cfg(feature = "z3")]
fn format_counterexample_block(cx: &str) -> String {
    crate::verifier_z3::format_counterexample_block(cx)
}
#[cfg(not(feature = "z3"))]
fn format_counterexample_block(_cx: &str) -> String {
    String::new()
}
    ...
    pub fn check_node(&mut self, node: &Node) -> Result<Type, String> {
        match node {
            ...
            Node::Function {
                ...
                // RES-223: tag each clause with whether it's an `ensures`
                // so we can produce a distinct diagnostic format for
                // postcondition violations (multi-line counterexample block).
                let requires_len = requires.len();
                for (decl_idx, clause) in requires.iter().chain(ensures.iter()).enumerate() {
                    let is_ensures = decl_idx >= requires_len;
                    ...
                    match verdict {
                        Some(false) => {
                            // RES-223: `ensures` postcondition failures use
                            // a multi-line counterexample block so the user
                            // can read the witness variable-by-variable.
                            // `requires` failures keep the existing inline
                            // format to avoid any behaviour change there.
                            if is_ensures {
                                let base = format!(
                                    "fn {}: postcondition can never hold (statically false `ensures` clause)",
                                    name
                                );
                                return Err(match decl_counterexample {
                                    Some(cx) => {
                                        let block = format_counterexample_block(&cx);
                                        if block.is_empty() {
                                            base
                                        } else {
                                            format!("{}\n{}", base, block)
