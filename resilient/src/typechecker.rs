...
impl TypeChecker {
    ...
    pub fn check_node(&mut self, node: &Node) -> Result<Type, String> {
        match node {
            ...
            // RES-208 (concurrency-preview): emit a W0001 warning to
            // stderr and return Type::Void so downstream phases see a
            // well-typed (if unexecutable) program.  The interpreter
            // will reject these nodes at eval time with a clear
            // "scheduler not wired up" message.
            ...
            Node::ActorDecl { .. } | Node::Spawn { .. } | Node::Send { .. } | Node::Recv { .. } => {
                eprintln!(
                    "warning[W0001]: concurrency-preview feature used — \
                     `actor`/`spawn`/`send`/`recv` syntax is not stable \
                     and cannot be executed yet (RES-208)"
                );
                Ok(Type::Void)
            }
        }
