...
impl TypeChecker {
    ...
    pub fn check_node(&mut self, node: &Node) -> Result<Type, String> {
        match node {
            ...
            // RES-222: `invariant <expr>;` — the condition must be Bool.
            Node::InvariantStatement { condition, .. } => {
                let cond_type = self.check_node(condition)?;
                if cond_type != Type::Bool && cond_type != Type::Any {
                    return Err(format!(
                        "Invariant condition must be a boolean, got {}",
                        cond_type
                    ));
                }
                Ok(Type::Void)
            ...
            // RES-221: each interpolated sub-expression must type-check;
            // the whole expression evaluates to a String.
            Node::StringInterpolation { parts, .. } => {
                for part in parts {
                    if let crate::StringPart::Expr(expr) = part {
...
fn check_body_purity(
    ...
    match node {
        ...
        // RES-222: invariant statement — its condition may contain calls.
        Node::InvariantStatement { condition, .. } => {
            check_body_purity(condition, fn_name, pure_fns)?;
        }
        Node::LiveBlock { .. } => {
...
fn body_reaches_io(node: &Node, effects: &std::collections::HashMap<String, bool>) -> bool {
    match node {
        ...
        // RES-222: invariant condition may contain IO (rare but legal in non-pure fns).
        Node::InvariantStatement { condition, .. } => body_reaches_io(condition, effects),
        Node::LiveBlock {
            body, invariants, ..
        } => {
