...
impl TypeChecker {
    ...
    fn bind_enum_type_parameter(
        ...
        actual: &Type,
        position: usize,
        bindings: &mut HashMap<String, (Type, usize)>,
    ) -> Result<(), String> {
        // Any carries no usable inference information. Leaving it
        // unbound preserves the existing conservative behavior when a
        // dynamic expression shares a payload type parameter with a
        // concrete expression.
        if matches!(actual, Type::Any) {
            return Ok(());
        }
        if let Some((existing, existing_position)) = bindings.get(parameter) {
            if !self.type_satisfies(actual, existing) || !self.type_satisfies(existing, actual) {
                return Err(format!(
                    "Constructor {}::{}: type parameter {} inferred as both {} (payload position {}) and {} (payload position {})",
                    enum_name,
                    variant_name,
                    parameter,
                    existing,
            ...
            bindings.insert(parameter.to_string(), (actual.clone(), position));
        }
        Ok(())
    }
    ...
    pub fn check_program_with_source(
        ...
        match program {
            Node::Program(statements) => {
                ...
                // RES-3858: the former `ai_generated` pass is folded in
                // here — the tag is pure provenance metadata now.
                ...
                // RES-1623: validate property-test declarations and their
                // call sites when the attribute is present.
                ...
                // RES-2578: validate `-> !` (never type) function declarations.
                crate::never_type::check(program, source_path)?;
    ...
    fn check_node_inner(&mut self, node: &Node) -> Result<Type, String> {
        match node {
            ...
            Node::StructLiteral {
                ...
                // RES-4451: a repeated direct type parameter in a generic
                // enum's named payload must resolve to one compatible type
                // across every field in the same constructor call.
                if let Some((type_name, variant_name)) = name.rsplit_once("::")
                    && let Some(type_params) = self.enum_type_params.get(type_name).cloned()
                    && let Some(variants) = self.enum_decls.get(type_name).cloned()
                    && let Some(variant) = variants.iter().find(|v| v.name == variant_name)
                    && let crate::EnumPayload::Named(declared) = &variant.payload
                {
                    let mut bindings: HashMap<String, (Type, usize)> = HashMap::new();
                    for (index, (field_name, field_expr)) in fields.iter().enumerate() {
                        let actual_ty = self.check_node(field_expr)?;
                        let Some(declared_field) =
                            declared.iter().find(|field| field.name == *field_name)
                        else {
                            return Err(format!(
                        ...
                        if let Some(param) = type_params
                            .iter()
                            .find(|param| param.as_str() == declared_field.ty.as_str())
                        {
                            self.bind_enum_type_parameter(
                                type_name,
                                variant_name,
                                param,
                                &actual_ty,
                        ...
                        } else if let Ok(expected_ty) = self.parse_type_name(&declared_field.ty)
                            && !self.type_satisfies(&actual_ty, &expected_ty)
                        {
                            return Err(format!(
                                "Constructor {}::{}: field {} has type {}, expected {}",
                                type_name, variant_name, field_name, actual_ty, expected_ty
                            ));
                        }
                    }
            ...
            Node::CallExpression {
                    ...
                    && let Some(idx) = name.rfind("::")
                {
                        ...
                        && let crate::EnumPayload::Tuple(declared) = &v.payload
                    {
                        ...
                        let mut bindings: HashMap<String, (Type, usize)> = HashMap::new();
                        for (index, (arg, type_str)) in
                            arguments.iter().zip(declared.iter()).enumerate()
                        {
                            let arg_ty = self.check_node(arg)?;
                            if let Some(ref params) = tp
                                && let Some(param) = params.iter().find(|p| p == &type_str)
                            {
                                self.bind_enum_type_parameter(
                                    type_name,
                                    variant_name,
...
fn check_program_effects(
    ...
    for stmt in statements {
        if let Node::Function {
            ...
            && effects.pure
            && let Err(reason) = check_body_effects(body, &fn_effects, parameters, statements, body)
        {
            let (line, col) = (stmt.span.start.line, stmt.span.start.column);
            return Err(if line == 0 {
                format!("pure fn `{}`: {}", name, reason)
...
fn check_body_effects(
    ...
    match node {
        Node::Block { stmts, .. } => {
            for s in stmts {
                check_body_effects(s, fn_effects, linear_params, statements, caller_body)?;
            }
        }
        Node::LetStatement { value, .. } | Node::StaticLet { value, .. } => {
            check_body_effects(value, fn_effects, linear_params, statements, caller_body)?;
        }
        Node::ReturnStatement { value: Some(v), .. } => {
            check_body_effects(v, fn_effects, linear_params, statements, caller_body)?
        }
        Node::ReturnStatement { value: None, .. } => {}
        Node::IfStatement {
            condition,
            ...
            if let Some(a) = alternative {
                check_body_effects(a, fn_effects, linear_params, statements, caller_body)?;
            }
        }
        Node::WhileStatement {
            condition, body, ..
            ...
            check_body_effects(body, fn_effects, linear_params, statements, caller_body)?;
        }
        Node::ForInStatement { iterable, body, .. } => {
            check_body_effects(iterable, fn_effects, linear_params, statements, caller_body)?;
            check_body_effects(body, fn_effects, linear_params, statements, caller_body)?;
        }
        ...
        Node::InfixExpression { left, right, .. } => {
            check_body_effects(left, fn_effects, linear_params, statements, caller_body)?;
            check_body_effects(right, fn_effects, linear_params, statements, caller_body)?;
        }
        Node::PrefixExpression { right, .. } => {
            check_body_effects(right, fn_effects, linear_params, statements, caller_body)?
        }
        Node::CallExpression {
            ...
            check_body_effects(function, fn_effects, linear_params, statements, caller_body)?;
            return Err(
                "cannot call indirect/method callee from pure context (effect unknown)".to_string(),
            );
        }
        Node::FieldAccess { target, .. } => {
            check_body_effects(target, fn_effects, linear_params, statements, caller_body)?
        }
        Node::FieldAssignment { target, value, .. } => {
            check_body_effects(target, fn_effects, linear_params, statements, caller_body)?;
            check_body_effects(value, fn_effects, linear_params, statements, caller_body)?;
        ...
        Node::IndexExpression { target, index, .. } => {
            check_body_effects(target, fn_effects, linear_params, statements, caller_body)?;
            check_body_effects(index, fn_effects, linear_params, statements, caller_body)?;
        }
        Node::IndexAssignment {
            target,
            index,
        ...
        Node::ArrayLiteral { items, .. } => {
            for i in items {
                check_body_effects(i, fn_effects, linear_params, statements, caller_body)?;
            }
        }
        Node::StructLiteral { fields, base, .. } => {
            if let Some(b) = base {
                check_body_effects(b, fn_effects, linear_params, statements, caller_body)?;
            }
            for (_, v) in fields {
                check_body_effects(v, fn_effects, linear_params, statements, caller_body)?;
            }
        ...
        Node::Match {
            ...
            for (_pat, guard, arm_body) in arms {
                if let Some(g) = guard {
                    check_body_effects(g, fn_effects, linear_params, statements, caller_body)?;
                }
                check_body_effects(arm_body, fn_effects, linear_params, statements, caller_body)?;
            }
        }
        Node::ExpressionStatement { expr, .. } => {
            check_body_effects(expr, fn_effects, linear_params, statements, caller_body)?
        }
        Node::TryExpression { expr, .. } => {
            check_body_effects(expr, fn_effects, linear_params, statements, caller_body)?
        }
        Node::OptionalChain { object, access, .. } => {
            check_body_effects(object, fn_effects, linear_params, statements, caller_body)?;
            if let crate::ChainAccess::Method(_, args) = access {
                for a in args {
                    check_body_effects(a, fn_effects, linear_params, statements, caller_body)?;
                }
...
mod res2814_generic_enum_constructors {
    ...
    #[test]
    fn repeated_generic_tuple_payload_types_must_agree() {
        let err = check_err("enum Pair<T> { Both(T, T) }\nlet p = Pair::Both(1, \"text\")\n");
        assert!(err.contains("type parameter T"), "unexpected error: {err}");
        assert!(
            err.contains("position 1") && err.contains("position 2"),
            "unexpected error: {err}"
        );
    }

    #[test]
    fn repeated_generic_named_payload_types_must_agree() {
        let err = check_err(
            "enum Pair<T> { Both { left: T, right: T } }\nlet p = new Pair::Both { left: 1, right: \"text\" }\n",
        );
        assert!(err.contains("type parameter T"), "unexpected error: {err}");
        assert!(
            err.contains("position 1") && err.contains("position 2"),
    ...
    #[test]
    fn generic_enum_any_payload_does_not_conflict() {
        check_ok("enum Pair<T> { Both(T, T) }\nlet p = Pair::Both(receive(), \"text\")\n");
    }
}
