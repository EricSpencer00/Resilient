...
pub enum Type {
    ...
    #[allow(dead_code)]
    Var(u32),
    /// RES-127: fixed-arity product type — `(Int, String)`. Elements
    /// are positionally indexed (`.0`, `.1`, …). `()` is an alias for
    /// `Type::Void`; `(T,)` is a one-element tuple. The arity is
    /// fixed at the construction site — no variadic tuples.
    Tuple(Vec<Type>),
}
...
///
/// RES-127: two `Tuple` types are compatible when they have the same
/// arity AND every element pair is compatible.
fn compatible(a: &Type, b: &Type) -> bool {
    if matches!(a, Type::Any) || matches!(b, Type::Any) {
        return true;
    }
    if let (Type::Tuple(as_), Type::Tuple(bs)) = (a, b) {
        return as_.len() == bs.len() && as_.iter().zip(bs.iter()).all(|(x, y)| compatible(x, y));
    }
    a == b
}
...
impl TypeChecker {
    ...
    pub fn check_node(&mut self, node: &Node) -> Result<Type, String> {
        match node {
            ...
            // RES-127: `(a, b)` tuple literal. Walk each element;
            // return `Type::Tuple` of the inferred element types.
            Node::TupleLiteral { items, .. } => {
                let mut elem_types = Vec::with_capacity(items.len());
                for item in items {
                    elem_types.push(self.check_node(item)?);
                }
                Ok(Type::Tuple(elem_types))
            }

            // RES-127: `t.0`, `t.1`, … tuple positional access.
            // Returns the declared element type when the target is a
            // known `Tuple`; falls back to `Any` for unknown targets.
                ...
                match val_ty {
                    Type::Tuple(elems) => {
                        if names.len() != elems.len() {
                            return Err(format!(
                                "tuple destructure arity mismatch: pattern has {} names, value has {} elements",
                                names.len(),
                                elems.len()
                            ));
                        }
                    ...
                    Type::Any => {
                        // Unknown tuple type — bind all names as Any.
                        for name in names {
                    ...
                    other => {
                        return Err(format!("tuple destructure requires a tuple, got {}", other));
                    }
                }
                Ok(Type::Void)
            }
    ...
    fn parse_type_name_inner(&self, name: &str, seen: &mut Vec<String>) -> Result<Type, String> {
        // RES-127: `(T1, T2, ...)` tuple type — serialised by the parser
        // as the string `"(T1, T2)"`. Recognise the leading `(`, split
        // the inner list, and recurse for each element type.
        if name.starts_with('(') && name.ends_with(')') {
            ...
            // Split on top-level commas only (no nested parens yet — a
            // limitation fine for the core slice; generics/nested tuples
            // extend this in a follow-up).
            let parts = split_tuple_type_parts(inner);
            let elems: Result<Vec<Type>, String> = parts
                .iter()
                .map(|s| self.parse_type_name_inner(s.trim(), seen))
                .collect();
            let elems = elems?;
...
/// RES-127: split a comma-separated type list at the top level (respecting
/// nested parentheses). Used to parse `T1, T2` out of `(T1, T2)` without
/// being confused by a nested tuple like `(Int, (Bool, Float))`.
fn split_tuple_type_parts(s: &str) -> Vec<&str> {
    ...
    // Trailing segment (possibly empty after a trailing comma like `(T,)`).
