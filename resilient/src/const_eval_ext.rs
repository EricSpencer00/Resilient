...
mod contracts {
    ...
    const ARRAY_SORT_BY_FIELD: &[ExpectedShape] =
        &[ExpectedShape::ListStruct, ExpectedShape::String];
    ...
    fn shape_matches(actual: &Shape, expected: ExpectedShape) -> bool {
        match (actual, expected) {
            ...
            (Shape::List(Some(inner)), ExpectedShape::ListString) => {
                matches!(**inner, Shape::String)
            }
            (Shape::List(Some(inner)), ExpectedShape::ListNumber) => {
                matches!(**inner, Shape::Number)
            }
            (Shape::List(Some(inner)), ExpectedShape::ListStruct) => {
                matches!(**inner, Shape::Struct)
            }
    ...
    fn infer_shape(node: &Node, const_shapes: &HashMap<String, Shape>) -> Option<Shape> {
        match node {
            ...
            Node::StructLiteral { .. }
            | Node::MapLiteral { .. }
            | Node::NewtypeConstruct { .. } => Some(Shape::Struct),
