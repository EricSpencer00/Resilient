...
enum Token {
    ...
    Type,
    /// RES-208: `actor <Name> { ... }` — concurrency-preview keyword.
    /// Only recognised when the `concurrency-preview` feature is active;
    /// otherwise the lexer returns `Identifier("actor")`.
    #[cfg(feature = "concurrency-preview")]
    Actor,
    /// RES-208: `spawn <Name>` — concurrency-preview keyword.
    #[cfg(feature = "concurrency-preview")]
    Spawn,
    /// RES-208: `send <expr>, <expr>` — concurrency-preview keyword.
    #[cfg(feature = "concurrency-preview")]
    Send,
    /// RES-208: `recv <expr>` — concurrency-preview keyword.
    #[cfg(feature = "concurrency-preview")]
    Recv,
            ...
            _ => {
                if self.is_letter(self.ch) {
                    ...
                    return match ident.as_str() {
                        ...
                        // RES-208: concurrency-preview keywords. Only reserved
                        // when the feature flag is active so that user code can
                        // still use these names as identifiers in a standard
                        // build without any breaking change.
                        #[cfg(feature = "concurrency-preview")]
                        "actor" => Token::Actor,
                        #[cfg(feature = "concurrency-preview")]
                        "spawn" => Token::Spawn,
...
enum Node {
    ...
    /// RES-208 (concurrency-preview): `actor <Name> { <body> }`.
    /// Parse-only scaffolding. The interpreter rejects this node at
    /// eval time with a clear "concurrency-preview not yet executable"
    /// error until a scheduler is wired up in a follow-on ticket.
    #[cfg(feature = "concurrency-preview")]
    ActorDecl {
        name: String,
        body: Box<Node>,
    ...
    /// RES-208 (concurrency-preview): `spawn <Name>` expression.
    /// Returns an opaque actor reference (parse scaffolding only).
    #[cfg(feature = "concurrency-preview")]
    Spawn {
        actor_name: String,
        #[allow(dead_code)]
    ...
    /// RES-208 (concurrency-preview): `send <target>, <message>`.
    /// Sends a message value to an actor reference (parse scaffolding only).
    #[cfg(feature = "concurrency-preview")]
    Send {
        target: Box<Node>,
        message: Box<Node>,
    ...
    /// RES-208 (concurrency-preview): `recv <channel>` expression.
    /// Receives the next message from an actor or channel (parse scaffolding only).
    #[cfg(feature = "concurrency-preview")]
    Recv {
        channel: Box<Node>,
        #[allow(dead_code)]
...
impl Parser {
    ...
    fn parse_statement(&mut self) -> Option<Node> {
        match self.current_token {
            ...
            // RES-208: concurrency-preview keywords dispatch to their
            // dedicated parsers only when the feature flag is active.
            #[cfg(feature = "concurrency-preview")]
            Token::Actor => Some(self.parse_actor_decl()),
            #[cfg(feature = "concurrency-preview")]
            Token::Spawn => Some(self.parse_spawn_expression()),
            #[cfg(feature = "concurrency-preview")]
            Token::Send => Some(self.parse_send_statement()),
            #[cfg(feature = "concurrency-preview")]
            Token::Recv => Some(self.parse_recv_expression()),
            Token::Unknown(ch) => {
    ...
    /// RES-208 (concurrency-preview): parse `actor <Name> { <body> }`.
    ///
    /// Grammar: `actor IDENT { STMT* }`
    ///
    /// Emits `Node::ActorDecl`. The body is a `Node::Block` parsed with
    /// the same block parser used by fn bodies and live blocks. The
    /// interpreter refuses to evaluate this node at runtime until a
    /// scheduler is wired up.
    #[cfg(feature = "concurrency-preview")]
    fn parse_actor_decl(&mut self) -> Node {
        debug_assert!(matches!(self.current_token, Token::Actor));
        let kw_span = self.span_at_current();
        ...
        if self.current_token != Token::LeftBrace {
            let tok = self.current_token.clone();
            self.record_error(format!(
                "expected `{{` to open actor body for `{}`, found {}",
                name, tok
            ));
        }
        let body = self.parse_block_statement();
    ...
    /// RES-208 (concurrency-preview): parse `spawn <Name>` as an
    /// expression statement.
    ///
    /// Grammar: `spawn IDENT ;`
    ///
    /// Returns `Node::Spawn` wrapped in `Node::ExpressionStatement`.
    /// `spawn` is always a statement in top-level or block context.
    #[cfg(feature = "concurrency-preview")]
    fn parse_spawn_expression(&mut self) -> Node {
        debug_assert!(matches!(self.current_token, Token::Spawn));
        let kw_span = self.span_at_current();
        self.next_token(); // skip `spawn`

        let actor_name = match &self.current_token {
            Token::Identifier(n) => n.clone(),
            other => {
                self.record_error(format!(
                    "expected actor name after `spawn`, found {}",
                    other
                ));
                String::new()
            }
    ...
    /// RES-208 (concurrency-preview): parse `send <target>, <message>;`.
    ///
    /// Grammar: `send EXPR , EXPR ;`
    ///
    /// Emits `Node::Send`.
    #[cfg(feature = "concurrency-preview")]
    fn parse_send_statement(&mut self) -> Node {
        debug_assert!(matches!(self.current_token, Token::Send));
        let kw_span = self.span_at_current();
        self.next_token(); // skip `send`

        let target = self.parse_expression(0).unwrap_or(Node::IntegerLiteral {
            value: 0,
            span: span::Span::default(),
        });

        if self.current_token == Token::Comma || self.peek_token == Token::Comma {
            if self.current_token != Token::Comma {
                self.next_token();
            }
            self.next_token(); // skip `,`
        } else {
            self.record_error("expected `,` separating target and message in `send`".to_string());
        }

        let message = self.parse_expression(0).unwrap_or(Node::IntegerLiteral {
            value: 0,
            span: span::Span::default(),
        });
    ...
    /// RES-208 (concurrency-preview): parse `recv <channel>` as an
    /// expression statement.
    ///
    /// Grammar: `recv EXPR ;`
    ///
    /// Emits `Node::Recv` wrapped in `Node::ExpressionStatement`.
    #[cfg(feature = "concurrency-preview")]
    fn parse_recv_expression(&mut self) -> Node {
        debug_assert!(matches!(self.current_token, Token::Recv));
        let kw_span = self.span_at_current();
        self.next_token(); // skip `recv`

        let channel = self.parse_expression(0).unwrap_or(Node::IntegerLiteral {
            value: 0,
            span: span::Span::default(),
        });
...
impl Interpreter {
    ...
    fn eval(&mut self, node: &Node) -> RResult<Value> {
        match node {
            ...
            // RES-208 (concurrency-preview): the scheduler and message
            // queues are not wired up yet. Reject these nodes at eval
            // time with a clear diagnostic so the user knows this is a
            // parse-only preview, not a runtime-ready feature.
            #[cfg(feature = "concurrency-preview")]
            Node::ActorDecl { name, .. } => Err(format!(
                "concurrency preview: `actor {}` cannot be executed — \
                 the Resilient scheduler has not landed yet (RES-208). \
                 Parse scaffolding only; runtime support is a follow-on ticket.",
                name
            )),
            #[cfg(feature = "concurrency-preview")]
            Node::Spawn { actor_name, .. } => Err(format!(
                "concurrency preview: `spawn {}` cannot be executed — \
                 the Resilient scheduler has not landed yet (RES-208). \
                 Parse scaffolding only; runtime support is a follow-on ticket.",
                actor_name
            )),
            #[cfg(feature = "concurrency-preview")]
            Node::Send { .. } => Err("concurrency preview: `send` cannot be executed — \
                 the Resilient scheduler has not landed yet (RES-208). \
                 Parse scaffolding only; runtime support is a follow-on ticket."
                .to_string()),
            #[cfg(feature = "concurrency-preview")]
            Node::Recv { .. } => Err("concurrency preview: `recv` cannot be executed — \
                 the Resilient scheduler has not landed yet (RES-208). \
                 Parse scaffolding only; runtime support is a follow-on ticket."
                .to_string()),
        }
