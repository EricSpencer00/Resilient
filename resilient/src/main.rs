...
enum Node {
    ...
    /// RES-127: `(e1, e2, …)` tuple literal in expression position.
    /// A trailing comma distinguishes `(x,)` (single-element tuple)
    /// from `(x)` (parenthesised expression). `()` unit is handled as
    /// `Type::Void` at the call site; the parser never emits a zero-
    ...
    /// RES-127: `expr.N` positional tuple access (N is a non-negative
    /// integer literal). Parsed by extending the dotted-access path
    /// to accept `Token::IntLiteral` after the `.`.
    TupleIndex {
        tuple: Box<Node>,
        index: usize,
        span: span::Span,
    },
    /// RES-127: `let (x, y) = expr;` tuple destructuring. `names`
    /// holds the binding identifiers in source order; `value` is the
    /// RHS expression whose runtime value must be a `Value::Tuple` of
    /// the same arity.
    LetDestructureTuple {
        names: Vec<String>,
        value: Box<Node>,
...
impl Parser {
    ...
    fn parse_type_annotation(&mut self, ctx: &str) -> Option<String> {
        match &self.current_token {
            ...
            Token::LeftParen => {
                // RES-127: `(T1, T2)` tuple type annotation. Serialise
                // the whole thing into a single string of the form
                // `"(T1, T2)"` so it fits the existing `Option<String>`
                // slot. `parse_type_name_inner` in the typechecker knows
                // how to decode this format back to `Type::Tuple`.
                self.next_token(); // skip '('
                // `()` → void (unit tuple).
                if self.current_token == Token::RightParen {
                    self.next_token(); // skip ')'
                ...
                loop {
                    // Trailing `)` or EOF ends the list.
                    if matches!(self.current_token, Token::RightParen | Token::Eof) {
                        break;
                    }
                    match self.parse_type_annotation(ctx) {
                        Some(t) => parts.push(t),
                        None => break,
                    }
                    if self.current_token == Token::Comma {
    ...
    fn parse_let_statement(&mut self) -> Node {
        ...
        // RES-127: `let (x, y) = expr;` tuple destructuring.
        // A `(` immediately after `let` signals this form.
        if self.current_token == Token::LeftParen {
    ...
    /// RES-127: parse `let (x, y, …) = expr;`.
    /// On entry `current_token == LeftParen` (the `let` keyword has
    /// already been consumed). On exit `current_token` is on the last
    /// token of the value expression; the caller handles the trailing `;`.
    fn parse_let_destructure_tuple(&mut self, stmt_span: span::Span) -> Node {
        ...
        loop {
            ...
            names.push(name);
            self.next_token(); // past identifier
            if self.current_token == Token::Comma {
                self.next_token(); // skip ','
            } else {
                break;
        ...
        if self.current_token != Token::RightParen {
            let tok = self.current_token.clone();
            self.record_error(format!(
                "Expected `)` to close tuple destructure, found {}",
                tok
            ));
        } else {
            self.next_token(); // skip ')'
    ...
    fn parse_expression(&mut self, precedence: u8) -> Option<Node> {
        ...
        let mut left_expr = match &self.current_token {
            ...
            Token::LeftParen => {
                // RES-127: distinguish tuple literals from parenthesised
                // expressions.
                //   `(a, b)` → TupleLiteral (two or more items)
                //   `(x,)`  → TupleLiteral (singleton — trailing comma)
                //   `(x)`   → parenthesised expression (no trailing comma)
                //   `()`    → unit — leave for the caller; return Void
                //
                // The tricky part: `parse_expression` advances
                // `current_token` to the last token OF the sub-expression
                // and the NEXT token (after all infix operators are folded)
                // lands on `peek_token`. We mirror how `parse_call_arguments`
                // works — peek for `,` after each item and advance into the
                // next element only when we see one.
                let paren_span = self.span_at_current();
                self.next_token(); // skip '('

                // Empty parens `()` — unit/void.
                if self.current_token == Token::RightParen {
                    // Zero-element TupleLiteral; typechecker maps it to Void.
                    Some(Node::TupleLiteral {
                        items: Vec::new(),
                        span: paren_span,
                    })
                } else {
                    // Parse the first expression (current_token moves to the
                    // last token of `first`; peek_token is the token AFTER it).
                    ...
                    if self.peek_token == Token::Comma {
                        // Tuple literal: advance to ',' then continue.
                        self.next_token(); // current_token = ','
                        let mut items = vec![first];
                        loop {
                            // Trailing comma followed by ')' ends the tuple.
                            ...
                            match self.parse_expression(0) {
                                ...
                                None => {
                                    let tok = self.current_token.clone();
                                    self.record_error(format!(
                                        "Expected expression in tuple literal, found {}",
                                        tok
                                    ));
                                    break;
                                }
                            }
                            // peek_token is now ',' or ')' or something else.
                            if self.peek_token == Token::Comma {
                                self.next_token(); // move to ','
                            // loop continues — next iteration peeks again
                            } else if self.peek_token == Token::RightParen {
                                self.next_token(); // move to ')'
                                break;
                            } else {
                                let tok = self.peek_token.clone();
                                self.record_error(format!(
                                    "Expected `,` or `)` in tuple literal, found {}",
                                    tok
                                ));
                                break;
                            }
                        }
                        // current_token is ')' here.
                        if self.current_token != Token::RightParen {
                            let tok = self.current_token.clone();
                            self.record_error(format!(
                        ...
                        // Parenthesised expression `(x)`.
                        // peek_token after `first` must be ')'.
                        if self.peek_token != Token::RightParen {
                            let tok = self.peek_token.clone();
                            self.record_error(format!(
    ...
    fn parse_field_access(&mut self, target: Node) -> Option<Node> {
        ...
        self.next_token(); // skip '.'
        // RES-127: `t.0`, `t.1`, … positional tuple access. If the
        // token after `.` is a non-negative integer literal, emit a
        // `TupleIndex` node instead of `FieldAccess`.
        if let Token::IntLiteral(n) = &self.current_token {
            ...
            let tok = self.current_token.clone();
            self.record_error(format!(
                "Tuple index must be a non-negative integer, found {}",
                tok
            ));
            return Some(target);
        }
...
enum Value {
    ...
    Array(Vec<Value>),
    /// RES-127: fixed-arity product value — `(1, "hello")`. Elements
    /// are accessed positionally via `.0`, `.1`, etc. VM/JIT lowering
    /// is deferred; the interpreter works directly on the Vec.
    Tuple(Vec<Value>),
...
impl Interpreter {
    ...
    fn eval(&mut self, node: &Node) -> RResult<Value> {
        match node {
            ...
            // RES-127: tuple literal — evaluate each element.
            // A zero-element `TupleLiteral` (unit `()`) evaluates to `Void`.
            ...
            // RES-127: `t.N` positional tuple access.
            Node::TupleIndex { tuple, index, span } => {
                let tval = self.eval(tuple)?;
                match tval {
                    Value::Tuple(items) => {
                        if *index >= items.len() {
                            return Err(format!(
                    ...
                    other => Err(format!(
                        "{}:{}: `.{}` requires a tuple value, got {}",
                        span.start.line, span.start.column, index, other
                    )),
                }
            }
            // RES-127: `let (x, y) = expr;` tuple destructuring.
            Node::LetDestructureTuple { names, value, span } => {
                let val = self.eval(value)?;
                if matches!(val, Value::Return(_)) {
                    return Ok(val);
                }
                match val {
                        ...
                        if names.len() != items.len() {
                            return Err(format!(
                                "{}:{}: tuple destructure arity mismatch: \
                                 pattern has {} names, value has {} elements",
                                span.start.line,
                                span.start.column,
                                names.len(),
                                items.len()
                    ...
                    other => Err(format!(
                        "{}:{}: tuple destructure requires a tuple value, got {}",
                        span.start.line, span.start.column, other
                    )),
                }
            }
...
mod tests {
    ...
    // ── RES-127: tuple types + destructuring let ───────────────────────────

    /// `(1, 2).0` → 1
    #[test]
    fn tuple_index_first() {
        let result = eval_to_int("let t = (1, 2); t.0");
        assert_eq!(result, 1);
    }
    ...
    fn tuple_index_second() {
        let result = eval_to_int("let t = (1, 2); t.1");
        assert_eq!(result, 2);
    }

    /// Mixed-type tuple: `.1` returns the string element.
    #[test]
    fn tuple_mixed_index() {
        let src = r#"let t = (1, "hello"); t.1"#;
        let (program, errs) = parse(src);
        assert!(errs.is_empty(), "parse errors: {:?}", errs);
        let mut interp = Interpreter::new();
        match interp.eval(&program).unwrap() {
    ...
    fn tuple_destructure_let() {
        let result = eval_to_int("let (x, y) = (10, 20); x + y");
        assert_eq!(result, 30);
    }

    /// Single-element tuple `(42,)` round-trips as a tuple.
    #[test]
    fn tuple_singleton() {
        let src = "let t = (42,); t.0";
        let result = eval_to_int(src);
        assert_eq!(result, 42);
    }

    /// Function returning a tuple; caller destructures it.
    #[test]
    fn tuple_from_function() {
        let result = eval_to_int(
            "fn pair(int a, int b) { return (a, b); } \
             let (x, y) = pair(3, 4); x + y",
        );
        assert_eq!(result, 7);
    }

    /// Arity mismatch in destructure is a runtime error.
    #[test]
    fn tuple_destructure_arity_mismatch_runtime_error() {
        let src = "let (a, b, c) = (1, 2); a";
        let (program, errs) = parse(src);
        assert!(errs.is_empty(), "parse errors: {:?}", errs);
        let mut interp = Interpreter::new();
        let err = interp.eval(&program).unwrap_err();
    ...
    /// Typechecker catches arity mismatch statically.
    #[test]
    fn tuple_type_arity_mismatch_typecheck_error() {
        // `(Int, Bool)` vs `(Int, Int, Int)` is incompatible.
        let src = "let x: (int, bool) = (1, 2, 3);";
        let (program, errs) = parse(src);
        assert!(errs.is_empty(), "parse errors: {:?}", errs);
        let mut tc = typechecker::TypeChecker::new();
        ...
        assert!(
            result.is_err(),
            "expected type error for arity mismatch, got Ok"
        );
    }
}
