...
pub(crate) fn builtin_float_approx_eq(args: &[Value]) -> RResult<Value> {
    ...
    match args {
        [a, b, eps] => {
            ...
            if feps < 0.0 {
                return Err(format!(
                    "float_approx_eq: eps must be >= 0, got {feps}"
                ));
            }
...
pub(crate) fn builtin_round_to(args: &[Value]) -> RResult<Value> {
    let (x, n) = match args {
        ...
        [_, n] => {
            return Err(format!(
                "round_to: second argument must be an int, got {n}"
            ))
        }
        _ => {
            return Err(format!(
    ...
    if n < 0 {
        return Err(format!(
            "round_to: decimal places must be >= 0, got {n}"
        ));
    }
...
pub(crate) fn builtin_int_pow(args: &[Value]) -> RResult<Value> {
    match args {
        [Value::Int(base), Value::Int(exp)] => {
            if *exp < 0 {
                return Err(format!(
                    "int_pow: exponent must be >= 0, got {exp}"
                ));
            }
            Ok(Value::Int(base.wrapping_pow(*exp as u32)))
        }
        [Value::Int(_), e] => Err(format!(
            "int_pow: exponent must be an int, got {e}"
        )),
