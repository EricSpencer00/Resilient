...
pub(crate) fn builtin_result_map(
...
pub(crate) fn builtin_result_and_then(
    interp: &mut Interpreter,
    args: &[Value],
) -> RResult<Value> {
    let (r, f) = match args {
...
/// unchanged.
pub(crate) fn builtin_result_map_err(
    interp: &mut Interpreter,
    args: &[Value],
) -> RResult<Value> {
    let (r, f) = match args {
...
pub(crate) fn builtin_result_or_else(
    interp: &mut Interpreter,
    args: &[Value],
) -> RResult<Value> {
    let (r, f) = match args {
...
pub(crate) fn builtin_option_map(
    interp: &mut Interpreter,
    args: &[Value],
) -> RResult<Value> {
    let (o, f) = match args {
...
pub(crate) fn builtin_option_and_then(
    interp: &mut Interpreter,
    args: &[Value],
) -> RResult<Value> {
    let (o, f) = match args {
...
pub(crate) fn builtin_option_filter(
    interp: &mut Interpreter,
    args: &[Value],
) -> RResult<Value> {
    let (o, f) = match args {
    ...
    match o {
        Value::Option(Some(inner)) => {
            match interp.apply_function(f, vec![*inner.clone()])? {
...
pub(crate) fn builtin_option_or_else(
    interp: &mut Interpreter,
    args: &[Value],
) -> RResult<Value> {
    let (o, f) = match args {
