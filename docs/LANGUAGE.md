...
| Trait objects (`dyn Trait`, vtable dispatch) | Unimplemented / descoped for now | `lib.rs::parse_type_annotation` recognizes `dyn Trait` syntax and emits a dedicated diagnostic ("trait objects ... not supported ... use a generic type parameter with a trait bound instead", A-E3/RES-3933) rather than a confusing downstream parse error; no vtable, no `dyn`-typed value representation in any of the three backends | Not merely deprioritized — there is no dispatch path for this today, only a clear rejection at parse time. Do not document it as Experimental (which implies active work); it is a design idea only, tracked in [#4068](https://github.com/EricSpencer00/Resilient/issues/4068). |
